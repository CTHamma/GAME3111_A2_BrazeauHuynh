...
#include "../../../GitHub/DirectX/Common/d3dApp.h"
#include "../../../GitHub/DirectX/Common/MathHelper.h"
#include "../../../GitHub/DirectX/Common/UploadBuffer.h"
#include "../../../GitHub/DirectX/Common/GeometryGenerator.h"
...
void BlendApp::UpdateMainPassCB(const GameTimer& gt)
{
	...
	//point lights in towers
	mMainPassCB.Lights[1].Position = { -25.0f, 20.5f, -25.0f };
	mMainPassCB.Lights[1].Strength = { 35.0f, 20.0f, 0.0f };

	mMainPassCB.Lights[2].Position = { -25.0f, 20.5f, 25.0f };
	mMainPassCB.Lights[2].Strength = { 35.0f, 20.0f, 0.0f };
...
void BlendApp::LoadTextures()
{
	...
	auto abstractTex = std::make_unique<Texture>();
	abstractTex->Name = "abstractTex";
	abstractTex->Filename = L"Textures/abstract.dds";
	ThrowIfFailed(DirectX::CreateDDSTextureFromFile12(md3dDevice.Get(),
...
void BlendApp::BuildDescriptorHeaps()
{
	...
	srvDesc.Format = abstractTex->GetDesc().Format;
	md3dDevice->CreateShaderResourceView(abstractTex.Get(), &srvDesc, hDescriptor);
...
void BlendApp::BuildShapeGeometry()
{
	...
	GeometryGenerator::MeshData cone = geoGen.CreateCylinder(1.0f, 0.0f, 3.0f, 20, 20);
...
{
	GeometryGenerator geoGen;
	GeometryGenerator::MeshData cube = geoGen.CreateBox(8.0f, 8.0f, 8.0f, 3);

	std::vector<Vertex> vertices(cube.Vertices.size());
	for (size_t i = 0; i < cube.Vertices.size(); ++i)
	{
		auto& p = cube.Vertices[i].Position;
		vertices[i].Pos = p;
...
void BlendApp::BuildMaterials()
{
	...
	abstractmat->DiffuseSrvHeapIndex = 2;
	abstractmat->DiffuseAlbedo = XMFLOAT4(1.0f, 1.0f, 1.0f, 1.0f);
	...
	wooden->Name = "wooden";
	wooden->MatCBIndex = 5;
	wooden->DiffuseSrvHeapIndex = 5;
	wooden->DiffuseAlbedo = XMFLOAT4(1.0f, 1.0f, 1.0f, 1.0f);
	wooden->FresnelR0 = XMFLOAT3(0.1f, 0.1f, 0.1f);
	wooden->Roughness = 0.25f;
...
void BlendApp::BuildRenderItems()
{
	...
	mRitemLayer[(int)RenderLayer::AlphaTested].push_back(cubeRitem.get());
	...
	XMStoreFloat4x4(&box6Ritem->World, XMMatrixScaling(10.0f, 2.0f, 12.5f) * XMMatrixTranslation(0.0f, 1.0f, 33.5f));

	box6Ritem->ObjCBIndex = 8;
	box6Ritem->Mat = mMaterials["wooden"].get();
	...
	XMStoreFloat4x4(&gridRitem->World, XMMatrixScaling(6.0f, 1.0f, 4.0f) * XMMatrixTranslation(0.0f, 0.0f, 0.0f));

	gridRitem->ObjCBIndex = 9;
	...
	for (int i = 0; i < 2; ++i)
	{
		...
		XMMATRIX leftCylWorld = XMMatrixScaling(8.0f, 5.0f, 8.0f) * XMMatrixTranslation(-25.0f, 7.5f, -25.0f + i * 50.0f);
		XMMATRIX rightCylWorld = XMMatrixScaling(8.0f, 5.0f, 8.0f) * XMMatrixTranslation(+25.0f, 7.5f, -25.0f + i * 50.0f);

		XMMATRIX leftConeWorld = XMMatrixScaling(5.0f, 5.0f, 5.0f) * XMMatrixTranslation(-25.0f, 22.5f, -25.0f + i * 50.0f);
		XMMATRIX rightConeWorld = XMMatrixScaling(5.0f, 5.0f, 5.0f) * XMMatrixTranslation(+25.0f, 22.5f, -25.0f + i * 50.0f);
		...
		leftConeRitem->Mat = mMaterials["abstractmat"].get();
		...
		rightConeRitem->Mat = mMaterials["abstractmat"].get();
		...
		mRitemLayer[(int)RenderLayer::AlphaTested].push_back(rightCylRitem.get());
		mRitemLayer[(int)RenderLayer::AlphaTested].push_back(leftConeRitem.get());
		mRitemLayer[(int)RenderLayer::AlphaTested].push_back(rightConeRitem.get());
